...
class BooksApp extends React.Component {
  ...
  updateQuery = (query) => {
    ...
    :BooksAPI.search(query).then((response)=>{response.shelf:{}})
      ()=>({response.map(
        (bookres)=>({this.state.books.map(
          (booknow)=>({booknow.id===bookres.id?bookres.shelf=booknow.shelf:'none'}))}))})}))

      //this.setState({showingBooks:
        //response.map(name => {
      //this.state.books.map((booknow)=>({name.id===booknow.id?name.shelf=booknow.shelf}))
      //name.shelf='none'; return name}).map((bookmatch)=>({}))}); console.log(response.length>0?'yes':'no')})
        ...
        <Route path='/search' render={() => (
          <div className="search-books">
            ...
            <div className="search-books-results">
              ...
              <Bookshelf title={query} onChange={this.update_shelf} books={showingBooks}
              //{this.showingBooks}
              />
            </div>
          </div>
